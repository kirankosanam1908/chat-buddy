...
const ChatList = () => {
  ...
  const handleSelectedChat = (name) => {
    setSelectedChat(name);
  };

  return (
    <div className="hidden h-full flex-col xl:flex xl:w-1/4 ">
      <div className="flex flex-row sticky border-b border-stroke dark:border-strokedark px-6  py-7.5">
      ...
      <div className="flex max-h-full flex-col overflow-auto p-5 ">
        <form className="sticky mb-7 flex flex-row">
          <input
            type="text"
            placeholder="Search...."
            className="w-full rounded border border-stroke bg-gray-2  py-2.5 pl-5 pr-10 text-sm outline-none  focus:border-primary dark:border-strokedark dark:bg-boxdark-2 dark:text-bodydark1"
          />
          <button className="absolute right-4 top-1/2 -translate-y-1/2 dark:text-bodydark">
          ...
          {List.map((eachItem) => (
            <div
              ...
              className={`flex cursor-pointer items-center rounded px-4 py-2  dark:hover:bg-strokedark ${
                selectedChat === eachItem.imgSrc
                  ? "bg-gray dark:bg-boxdark-2"
                  : "hover:bg-gray-2 dark:hover:bg-boxdark-2/90"
              }  `}
              onClick={() => {
                handleSelectedChat(eachItem.imgSrc);
              }}
            >
              <div className="relative mr-3.5 w-11 max-w-11 rounded-full">
                <img
                  src={eachItem.imgSrc}
                  alt="profile"
